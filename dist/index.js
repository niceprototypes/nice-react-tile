 ...
 */
const ContentMain = ({ title, titleProps, contentCenter, description, descriptionProps, mode, }) => (jsxRuntime.jsxs(Flex, { direction: "column", gap: descriptionProps.size, children: [title && (jsxRuntime.jsx(Typography, { as: "h3", weight: "semibold", mode: mode, ...titleProps, children: title })), contentCenter, description && (Array.isArray(description)
...
const TileContent = ({ children, contentTop, contentCenter, title, titleProps, description, descriptionProps, align, mode, gap, }) => {
    ...
    return (jsxRuntime.jsxs(Flex, { direction: "column", gap: "none", children: [contentTop, hasContentMain && (jsxRuntime.jsx(ContentMain, { title: title, titleProps: titleProps, contentCenter: contentCenter, description: description, descriptionProps: descriptionProps, mode: mode })), children && (jsxRuntime.jsx(Flex, { direction: "column", spacing: gap ? `${gap} none none` : undefined, children: children }))] }));
};
...
const Tile = ({ children, contentTop, contentCenter, contentLeft: TileLeft, contentRight: TileRight, title, titleProps, description, descriptionProps, spacing, maxWidthMedium, maxWidthLarge, alignItems, justifyContent, gap, backgroundImage, backgroundColor, backgroundPosition = "center", backgroundSize = "cover", backgroundAttachment = "fixed", foregroundColor, mode, className, style, }) => {
